...
const FormularioModerno = ({ tipo, dados, onChange, erros = {}, icone, cor }) => {
  const handleChange = (campo, valor) => {
    onChange({ ...dados, [campo]: parseFloat(valor) || 0 });
  };

  const campos = tipo === "consorcio" 
    ? [
        { 
          id: "valorBem", 
          label: "Valor do Bem", 
  ...
  return (
    <Card className={`border-2 ${cor} hover:scale-[1.01] transition-all duration-300 animate-fade-in`}>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`p-3 rounded-full ${tipo === "consorcio" ? "bg-green-100" : "bg-blue-100"}`}>
      ...
      <CardContent className="space-y-4">
        {campos.map((campo) => {
          ...
          return (
            <div key={campo.id} className="space-y-2">
              <Label htmlFor={campo.id} className="text-gray-700 flex items-center gap-2">
              ...
              <div className="relative">
                {campo.id.includes("valor") || campo.id === "entrada" || campo.id === "lance" ? (
                ...
                <Input
                  ...
                  className={`${
                    (campo.id.includes("valor") || campo.id === "entrada" || campo.id === "lance")
                      ...
                      ? "pr-12"
                      : ""
                  } ${erros[campo.id] ? "border-red-500 focus-visible:ring-red-500" : ""}`}
                  inputMode={campo.inputMode || "numeric"}
                  min="0"
                  step={campo.id === "taxaAnual" ? "0.1" : campo.id === "prazoMeses" ? "1" : "1000"}
                />
                {campo.suffix && (
        ...
        <div className={`${tipo === "consorcio" ? "bg-green-50 border-green-200" : "bg-blue-50 border-blue-200"} border rounded-lg p-4 mt-6`}>
          <h4 className={`font-semibold ${tipo === "consorcio" ? "text-green-800" : "text-blue-800"} mb-2 text-sm flex items-center gap-2`}>
            <Info className="w-4 h-4" />
            {tipo === "consorcio" ? "Taxas Aplicadas" : "Sistema de Cálculo"}
          </h4>
          <ul className={`${tipo === "consorcio" ? "text-green-700" : "text-blue-700"} text-xs space-y-1`}>
            {tipo === "consorcio" ? (
              <>
